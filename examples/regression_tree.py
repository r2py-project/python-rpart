...
fig.savefig(os.path.join(HERE, "cptable.pdf"))
plt.close(fig)
...
plot_rpart(tree_pruned, ax=ax)
text_rpart(tree_pruned, ax=ax)
ax.set_title("Pruned Regression Tree")
fig.savefig(os.path.join(HERE, "pruned.pdf"))
plt.close(fig)
...
text_rpart(tree_pruned, ax=ax)
ax.set_title("Pruned Regression Tree")
fig.savefig(os.path.join(HERE, "bootstraped.pdf"))
plt.close(fig)
