...
def residuals_rpart(object: dict[str, Any], type: str = 'usual', **kwargs: Any) -> np.ndarray[Any, np.dtype[np.float64]]:
    # Validate that object is an rpart model
    ...
    # Convert 1-based where to 0-based indices
    where: np.ndarray[Any, np.dtype[np.int64]] = np.asarray(object['where'], dtype=np.int64)
    ...
    if method == 'class':
        ...
        if type == 'usual':
            ...
            yhat_int: np.ndarray[Any, np.dtype[np.int64]] = frame['yval'].values[where_0].astype(np.int64)
            ...
            resid: np.ndarray[Any, np.dtype[np.float64]] = loss[y_int - 1, yhat_int - 1]
        else:
            # frame$yval2[object$where, 1L+nclass+1L:nclass] -> columns nclass+1 : 2*nclass+1
            yval2: np.ndarray[Any, np.dtype[np.float64]] = np.asarray(frame['yval2'].values[where_0], dtype=np.float64)
            yprob: np.ndarray[Any, np.dtype[np.float64]] = yval2[:, nclass + 1: 2 * nclass + 1]
        ...
        lambda_: np.ndarray[Any, np.dtype[np.float64]] = np.asarray(frame['yval'].values[where_0], dtype=np.float64)
        ...
        resid = np.asarray(y, dtype=np.float64) - np.asarray(frame['yval'].values[where_0], dtype=np.float64)
    # names(resid) <- names(y): propagate index if y is a pandas Series
