...
def predict_rpart(object: dict[str, Any], newdata: pd.DataFrame | None = None, type: str | None = None, na_action: Any = None) -> np.ndarray[Any, np.dtype[np.float64]] | pd.DataFrame | pd.Categorical:
    ...
    ylevels: list[str] | None = object.get('ylevels', None)
    if ylevels is None:
        # Also check as attribute-style storage
        ylevels = object.get('_attr_ylevels', None)
    ...
    if type == 'vector' or (type == 'matrix' and 'yval2' not in frame.columns):
        ...
        yval2: np.ndarray[Any, np.dtype[np.float64]] = frame['yval2'].values if hasattr(frame['yval2'], 'values') else np.array(frame['yval2'].tolist())
        ...
        yval2_all: np.ndarray[Any, np.dtype[np.float64]] = frame['yval2'].values if hasattr(frame['yval2'], 'values') else np.array(frame['yval2'].tolist())
    ...
    # naresid stub: if newdata is None and na_action recorded, restore NAs
    # Here we stub as returning pred directly (naresid is a no-op stub)
    if newdata is None and object.get('na.action') is not None:
        pass  # stub: naresid(object['na.action'], pred) -> return pred unchanged
    return pred
